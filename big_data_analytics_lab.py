from pyspark.sql import SparkSession
from pyspark.sql.types import StructType, StructField, IntegerType, StringType, DoubleType
from pyspark.sql import functions as F
from pyspark.sql.window import Window

# ============================================================
# Spark Session
# ============================================================
spark = SparkSession.builder \
    .appName("BigDataAnalyticsLab") \
    .getOrCreate()

# ============================================================
# Dataset
# ============================================================
transactions = [
    (1,  "T001", "Alice",   "North", "Electronics", 899.99, 2, "2024-01-05 10:30:00", "credit_card"),
    (2,  "T002", "Bob",     "South", "Clothing",     45.00, 3, "2024-01-06 11:00:00", "cash"),
    (3,  "T003", "Charlie", "East",  "Electronics", 199.50, 1, "2024-01-06 14:20:00", "debit_card"),
    (4,  "T004", "Alice",   "North", "Food",          12.50, 5, "2024-01-07 09:15:00", "cash"),
    (5,  "T005", "David",   "West",  "Electronics", 450.00, 1, "2024-01-08 16:45:00", "credit_card"),
    (6,  "T006", "Eve",     "South", "Food",          22.00, 4, "2024-01-08 18:00:00", "credit_card"),
    (7,  "T007", "Frank",   "North", "Clothing",     75.00, 2, "2024-01-09 13:30:00", "debit_card"),
    (8,  "T008", "Grace",   "East",  "Food",          33.00, 3, "2024-01-10 10:00:00", "cash"),
    (9,  "T009", "Heidi",   "West",  "Electronics", 600.00, 1, "2024-02-01 12:00:00", "credit_card"),
    (10, "T010", "Ivan",    "South", "Clothing",    110.00, 2, "2024-02-02 15:30:00", "debit_card"),
    (11, "T011", "Alice",   "North", "Electronics", 250.00, 1, "2024-02-03 09:00:00", "credit_card"),
    (12, "T012", "Bob",     "South", "Food",         18.00, 6, "2024-02-04 17:00:00", "cash"),
    (13, "T013", "Charlie", "East",  "Clothing",     95.00, 1, "2024-02-05 11:45:00", "credit_card"),
    (14, "T014", "David",   "West",  "Food",          8.50, 2, "2024-02-06 08:30:00", "debit_card"),
    (15, "T015", "Eve",     "South", "Electronics", 320.00, 1, "2024-02-07 14:00:00", "credit_card"),
    (16, "T016", "Frank",   "North", "Food",         55.00, 3, "2024-03-01 10:15:00", "cash"),
    (17, "T017", "Grace",   "East",  "Electronics", 780.00, 2, "2024-03-02 16:00:00", "credit_card"),
    (18, "T018", "Heidi",   "West",  "Clothing",    200.00, 1, "2024-03-03 12:30:00", "debit_card"),
    (19, "T019", "Ivan",    "South", "Food",         40.00, 5, "2024-03-04 09:45:00", "cash"),
    (20, "T020", "Alice",   "North", "Electronics", 999.99, 1, "2024-03-05 11:00:00", "credit_card"),
]

schema = StructType([
    StructField("id", IntegerType(), True),
    StructField("transaction_id", StringType(), True),
    StructField("customer", StringType(), True),
    StructField("region", StringType(), True),
    StructField("category", StringType(), True),
    StructField("unit_price", DoubleType(), True),
    StructField("quantity", IntegerType(), True),
    StructField("timestamp", StringType(), True),
    StructField("payment_method", StringType(), True),
])

# ============================================================
# Create DataFrame and Prepare Data
# ============================================================
df = spark.createDataFrame(transactions, schema)

df = df.withColumn("timestamp", F.to_timestamp("timestamp")) \
       .withColumn("revenue", F.col("unit_price") * F.col("quantity"))

print("\n=== Initial Data ===")
df.show(truncate=False)
df.printSchema()

# ============================================================
# Part 1: Descriptive Analytics
# ============================================================
print("\n=== Part 1: Descriptive Analytics ===")

df.describe(["unit_price", "quantity", "revenue"]).show()

revenue_by_category = df.groupBy("category") \
    .agg(F.round(F.sum("revenue"), 2).alias("total_revenue")) \
    .orderBy(F.desc("total_revenue"))

revenue_by_region = df.groupBy("region") \
    .agg(F.round(F.sum("revenue"), 2).alias("total_revenue")) \
    .orderBy(F.desc("total_revenue"))

print("\nRevenue by Category")
revenue_by_category.show()

print("\nRevenue by Region")
revenue_by_region.show()

# ============================================================
# Part 2: Diagnostic Analytics
# ============================================================
print("\n=== Part 2: Diagnostic Analytics ===")

monthly_revenue = df.withColumn("month", F.date_format("timestamp", "yyyy-MM")) \
    .groupBy("month") \
    .agg(F.round(F.sum("revenue"), 2).alias("total_revenue")) \
    .orderBy("month")

category_region_pivot = df.groupBy("region") \
    .pivot("category") \
    .agg(F.round(F.sum("revenue"), 2))

print("\nMonthly Revenue")
monthly_revenue.show()

print("\nCategory by Region Pivot")
category_region_pivot.show()

# ============================================================
# Part 3: Advanced Analytics with Window Functions
# ============================================================
print("\n=== Part 3: Advanced Analytics with Window Functions ===")

customer_window = Window.partitionBy("customer").orderBy("timestamp")
region_rank_window = Window.partitionBy("region").orderBy(F.desc("revenue"))

df_with_running_total = df.withColumn(
    "running_customer_revenue",
    F.round(
        F.sum("revenue").over(
            customer_window.rowsBetween(Window.unboundedPreceding, Window.currentRow)
        ),
        2
    )
)

ranked_transactions = df.withColumn(
    "region_revenue_rank",
    F.rank().over(region_rank_window)
)

print("\nRunning Customer Revenue")
df_with_running_total.select(
    "customer", "transaction_id", "timestamp", "revenue", "running_customer_revenue"
).show(truncate=False)

print("\nRegional Revenue Ranking")
ranked_transactions.select(
    "region", "transaction_id", "customer", "revenue", "region_revenue_rank"
).orderBy("region", "region_revenue_rank").show(truncate=False)

# ============================================================
# Part 4: Feature Engineering and RFM Scoring
# ============================================================
print("\n=== Part 4: Feature Engineering and RFM Scoring ===")

df_features = df.withColumn("hour", F.hour("timestamp")) \
    .withColumn("day_of_week", F.date_format("timestamp", "E")) \
    .withColumn("month", F.month("timestamp")) \
    .withColumn("is_weekend", F.when(F.dayofweek("timestamp").isin([1, 7]), 1).otherwise(0))

print("\nEngineered Features")
df_features.select(
    "transaction_id", "customer", "timestamp", "hour", "day_of_week", "month", "is_weekend", "revenue"
).show(truncate=False)

snapshot_date = df.agg(F.max("timestamp")).collect()[0][0]

rfm = df.groupBy("customer").agg(
    F.datediff(F.lit(snapshot_date), F.max("timestamp")).alias("recency"),
    F.count("transaction_id").alias("frequency"),
    F.round(F.sum("revenue"), 2).alias("monetary")
)

rfm = rfm.withColumn("r_score", F.ntile(4).over(Window.orderBy(F.col("recency").asc()))) \
         .withColumn("f_score", F.ntile(4).over(Window.orderBy(F.col("frequency").desc()))) \
         .withColumn("m_score", F.ntile(4).over(Window.orderBy(F.col("monetary").desc())))

print("\nRFM Scores")
rfm.show()

# ============================================================
# Part 5: Customer Segmentation
# ============================================================
print("\n=== Part 5: Customer Segmentation ===")

rfm_segments = rfm.withColumn(
    "segment",
    F.when((F.col("r_score") <= 2) & (F.col("f_score") <= 2) & (F.col("m_score") <= 2), "Champions")
     .when((F.col("r_score") >= 3) & (F.col("f_score") >= 3), "At Risk")
     .otherwise("Potential Loyalist")
)

print("\nCustomer Segments")
rfm_segments.orderBy("segment", "customer").show()

segment_counts = rfm_segments.groupBy("segment") \
    .count() \
    .orderBy(F.desc("count"))

print("\nSegment Counts")
segment_counts.show()

# ============================================================
# Part 6: Anomaly Detection
# ============================================================
print("\n=== Part 6: Anomaly Detection ===")

revenue_stats = df.agg(
    F.mean("revenue").alias("mean_revenue"),
    F.stddev("revenue").alias("stddev_revenue")
).collect()[0]

mean_revenue = revenue_stats["mean_revenue"]
stddev_revenue = revenue_stats["stddev_revenue"]

df_anomalies = df.withColumn(
    "z_score",
    F.round((F.col("revenue") - F.lit(mean_revenue)) / F.lit(stddev_revenue), 2)
)

anomalies = df_anomalies.filter(F.abs(F.col("z_score")) > 2)

print("\nAll Transactions with Z-Scores")
df_anomalies.select("transaction_id", "customer", "revenue", "z_score").show()

print("\nFlagged Anomalies")
anomalies.select("transaction_id", "customer", "region", "category", "revenue", "z_score").show()

# ============================================================
# Part 7: Save Output to Parquet
# ============================================================
print("\n=== Part 7: Save Output to Parquet ===")

output_path = "output/transactions_parquet"
df_features.write.mode("overwrite").parquet(output_path)

print(f"Parquet files written to: {output_path}")

spark.stop()